  ...
  useEffect(() => {
    ...
    if (code) {
      loginWithKakao(code)
        ...
        .catch((err) => {
          console.error("Kakao authentication failed:", err)
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-background">
      <div className="relative flex items-center justify-center">
        <div className="absolute size-16 rounded-full bg-[#FEE500]/20 animate-ping opacity-75" />
        <div className="relative flex size-14 items-center justify-center rounded-full bg-[#FEE500]/10 text-[#191919]">
          <Loader2 className="size-7 animate-spin" />
        </div>
      </div>
      <h2 className="mt-8 text-lg font-semibold text-foreground">카카오 로그인 처리 중...</h2>
      <p className="mt-2 text-sm text-muted-foreground">카카오 계정을 안전하게 연동하고 있습니다.</p>
    </div>
  )
}
